...
void CameraService::finishConnectLocked(const sp<BasicClient>& client,
    ...
    const char* packageName = String8(client->getPackageName()).string();
...
status_t CameraService::handleEvictionsLocked(const std::string& cameraId, int clientPid,
    ...
    DescriptorPtr clientDescriptor;
    {
        ...
        const char* packageNameStr = String8(packageName).string();
...
#ifdef CAMERA_NEEDS_CLIENT_INFO_LIB
    gVendorCameraProviderService->setPackageName(toString8(mClientPackageName).c_str());
#endif
